...
// отримуємо дані з сервера (замість URL потрібно вказати посилання на ваш сервер)
fetch('http://127.0.0.1:5000/api/v1/pharmacy/inventory')
  .then(response => response.json()) // перетворюємо відповідь сервера в об'єкт JavaScript
  .then(data => {
    // проходимось по масиву об'єктів і створюємо елементи
  ...
  .catch(error => console.error(error)); // обробляємо можливі помилки
