...
function Update() {
  ...
  const sortMedicineByDemandCount = (medicineList) => {
    return medicineList.sort((a, b) => {
      if (sortDirection === "asc") {
        return a.demand_count - b.demand_count;
      } else {
  ...
  const sortedMedicine = sortMedicineByDemandCount(filteredMedicine);
  ...
  return (
    <>
      ...
      <div className="delete">
        {
        sortedMedicine.length === 0 ? (<h2 className='fs-4 text-center'>No item</h2>) : (
        <table id="table" className="bordered">
          <thead>
            <tr>
              ...
              <th>Delete</th>
              <th>Update</th>
              <th onClick={handleSort}>Demand count</th>
            </tr>
          </thead>
          <tbody>
